            ...
            <el-col :xs="24" :sm="18" :md="12" :lg="10" :xl="6">
               <el-form
                ...
                <el-form-item label="店铺图片">
                    ...
                    <img v-if="form.shopImg" :src="'/api/download?name='+form.shopImg" class="logo">
  ...
  methods: {
    submitForm (formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.$axios.post('/api/admin/shop/set', this.form)
            .then(res => {
              this.$message({
  ...
  mounted () {
    this.$axios.get('/api/shop/get')
      .then(res => {
        ...
        this.form.shopImg = res.data.data.shopImg
        this.form.shopName = res.data.data.shopName
        this.form.shopIntro = res.data.data.shopIntro
        this.form.shopNotice = res.data.data.shopNotice
      })
      .catch(err => {
        console.log(err)
      })
